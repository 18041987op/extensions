...
create or replace view public.ro_audit
with (security_invoker = false) as
with active_ro as (
  select * from public.tekmetric_repair_orders
  where deleted_at is null and status in ('ESTIMATE','REPAIR_IN_PROGRESS')
),
job_agg as (
...
select
  ...
  coalesce(nullif(trim(e.full_name),''), nullif(trim(concat_ws(' ', e.first_name, e.last_name)),''), 'Sin asignar') as service_advisor,
  r.service_writer_id,
...
create or replace view public.sa_rollup
with (security_invoker = false) as
select
  service_advisor,
  service_writer_id,
  count(*) as active_ros,
  count(*) filter (where missing_vin or missing_miles or auth_job_without_tech
